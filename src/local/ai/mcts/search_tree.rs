...
impl<Spec: Mcts> SearchTree<Spec> {
    ...
    fn descend<'a, 'b>(
        ...
        let child: *const SearchNode<Spec> = choice.child.load(Ordering::Relaxed) as *const _;
        if !child.is_null() {
        ...
        {
            let child: *const SearchNode<Spec> = match choice.child.compare_exchange(
                null_mut(),
